	...
	<div class="Intelligent">
		...
		<div class="item" v-if="state.type == '1'">
			<div class="itemBox" style="width: 100%">
				...
				<el-button
					...
					@click="myIntelligentAnalysisParameter"
					>查询</el-button
				>
			</div>
		...
		<div class="item" v-if="state.type == '2'" style="height: 53vh; overflow: auto">
			...
			<div class="itemBox" style="width: 100%">
				...
				<el-input
					v-model="state1.id"
					placeholder="从1开始，无预置位传255"
					type="number"
					:min="0"
					:max="255"
					style="width: 60px"
				/>
				<span style="color: darkgray; padding-left: 10px">从1开始，无预置位传255</span>
			</div>
			<div class="itemBox" style="width: 100%">
				...
				<div style="position: relative; top: 3px">
					<el-icon
						...
						><ele-RemoveFilled
					/></el-icon>
					<el-icon size="20" style="cursor: pointer" @click="addAlarmTypeItem"
			...
			<div
				...
				v-for="(item, index) in state1.analyse.alarmTypes"
				:key="index"
			>
				<div class="itemBox" style="width: 100%">
					<div class="itemBox_title">枚举类型</div>
					<el-radio-group v-model="item.type" class="ml-4">
						<el-radio v-for="(value, key) in types1" :value="key" size="small" :key="key">{{
							types[value] || value
						}}</el-radio>
					</el-radio-group>
				</div>
				<div class="itemBox" style="width: 100%">
					...
					<el-input
						...
						placeholder="取值范围[1 - 100]，告警识别的置信度大于设置的阈值，才上报告警"
						type="number"
						:min="1"
						:max="100"
					/>
			...
			<div class="itemBox" style="width: 100%">
				...
				<div style="position: relative; top: 3px">
					<el-icon
						...
						><ele-RemoveFilled
					/></el-icon>
					<el-icon size="20" style="cursor: pointer" @click="addAlarmAreasItem"
						><ele-CirclePlusFilled
					/></el-icon>
				</div>
			</div>
			<div
				v-for="(item, index) in state1.analyse.alarmAreas"
				:key="index"
				style="width: 100%; border-bottom: 1px solid #ccc; padding-bottom: 10px"
			>
				<div class="itemBox">
				...
				<div class="itemBox">
					...
					<div style="position: relative; top: 3px">
						<el-icon
							...
							><ele-RemoveFilled
						/></el-icon>
						<el-icon size="20" style="cursor: pointer" @click="addPointsItem(index)"
							><ele-CirclePlusFilled
						/></el-icon>
					</div>
				</div>
				<div
					v-for="(item1, index1) in item.points"
					:key="index1"
			...
			<el-button
				...
				@click="mySetIntelligentAnalysisParameter"
				>设置</el-button
			>
		</div>
...
import {
	getChannelList,
	intelligentAnalysisParameter,
	setIntelligentAnalysisParameter,
	intelligentAnalysisTypes,
